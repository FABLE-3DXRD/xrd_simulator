...
new_labels = np.linspace( 2, 1 + len(grain_labels), len(grain_labels) ).astype(np.uint16)
print(new_labels)
...
save_as_vtk_voxel_volume("voroni_sample/voxelated_voroni_sample", labeled_volume, name='Labeled Grains', units=(1./len(g),1./len(g),1./len(g)), origin=(0.,0.,0.))
...
for label in new_labels:
    ...
    if np.sum(grain)!=0:
        ...
        print("Wrote "+"voroni_sample/grain_meshes/grain"+str(label).zfill(4)+".xdmf to file with "+str(mesh.cells_dict['tetra'].shape[0])+" elements in the mesh")
