...
class TestPolycrystal(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        det_corner_0 = np.array(
            [cls.detector_distance, -cls.detector_size / 2.0, -cls.detector_size / 2.0]
        )
        ...
        cls.detector = Detector(
            det_corner_0, det_corner_1, det_corner_2, pixel_size=cls.pixel_size
        )

        # Expensive mesh generation - only done once!
        # Use a small sphere with ~100-200 grains to keep memory manageable
        sphere_radius = 170.0
        cls.mesh = TetraMesh.generate_mesh_from_levelset(
            level_set=lambda x: np.dot(x, x) - sphere_radius,
            bounding_radius=1.1 * sphere_radius,
            max_cell_circumradius=0.138 * sphere_radius,
        )

        unit_cell = [4.926, 4.926, 5.4189, 90.0, 90.0, 120.0]
        sgname = "P3221"  # Quartz
        cls.phases = [Phase(unit_cell, sgname)]
        euler_angles = np.random.rand(cls.mesh.number_of_elements, 3) * 2 * np.pi
        cls.orientation = np.array(
            [tools.euler_to_u(ea[0], ea[1], ea[2]) for ea in euler_angles]
        )
        cls.element_phase_map = np.zeros((cls.mesh.number_of_elements,)).astype(int)

        w = cls.detector_size / 2.0  # full field illumination
    ...
    def test_diffract(self):
        ...
        peaks_dict = self.polycrystal.diffract(
            self.__class__.beam, motion, detector=self.__class__.detector, verbose=True
        )
        peaks_dict1 = self.polycrystal.diffract(
            self.__class__.beam, motion, detector=self.__class__.detector, verbose=False
        )
        ...
        self.assertTrue(
            np.allclose(diffraction_pattern_np, diffraction_pattern1_np),
            msg="Multiproccessing is broken",
        )

        # .. and the intensity should be scattered over the image
        w = int(self.__class__.detector_size / 5.0)
    ...
    def test_save_and_load(self):
        ...
        path = os.path.join(
            os.path.join(os.path.dirname(__file__), "data"), "my_polycrystal"
        )
    ...
    def test_dimension_handling(self):
        ...
        Polycrystal(
            mesh=self.__class__.mesh,
            orientation=self.__class__.orientation,
            strain=np.zeros((self.__class__.mesh.number_of_elements, 3, 3)),
            phases=self.__class__.phases,
            element_phase_map=self.__class__.element_phase_map,
        )
    ...
    def test_transformation(self):
        ...
        Rot_mat = motion.rotator.get_rotation_matrix(
            torch.tensor(time * rotation_angle)
        )
        Rot_mat = utils.ensure_numpy(Rot_mat)
        unit_vector = np.random.rand(
            3,
        )
        ...
        for i, strain in enumerate(strains):
            ...
            s2 = np.dot(new_unit_vector, np.dot(strain_lab_i, new_unit_vector))
