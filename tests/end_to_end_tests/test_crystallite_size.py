...
GAUSSIAN_SIGMA = (
    0.01  # Gaussian PSF sigma (very small to minimize instrumental broadening)
)
...
def find_peaks_in_pattern(radial_values, intensity, prominence_fraction=0.01):
    ...
    peaks, properties = find_peaks(
        intensity, prominence=min_prominence, height=min_prominence
    )
...
def fit_voigt_peak(x_data, y_data, center_guess, verbose=False):
    ...
    try:
        ...
        if verbose:
            print(
                f"  Pseudo-Voigt fit: center={center:.4f}°, FWHM={fwhm:.4f}°, "
...
class TestCrystalliteSize(unittest.TestCase):
    ...
    def _create_motion(self):
        ...
        return RigidBodyMotion(
            self.rotation_axis, self.rotation_angle, np.array([0.0, 0.0, 0.0])
        )
    ...
    def _plot_results(
        self, pattern_np, Q_result, I_result, theoretical_Q, found_Q, Q_max
    ):
        ...
        ax1.plot(
            center, center, "r+", markersize=15, markeredgewidth=2, label="Beam center"
        )
    ...
    def test_powder_pattern_with_scherrer(self):
        ...
        target_radius_um = (
            target_size_nm / 10000.0 / 2.0
        )  # nm -> Angstrom -> µm, diameter to radius
        target_volume = (4 / 3) * np.pi * target_radius_um**3
        print(f"    Actual average element volume: {avg_volume:.6e} µm³")
        print(
            f"    Effective crystallite size (from volume): {expected_size_nm:.1f} nm"
        )
        ...
        peaks_dict = polycrystal.diffract(
            beam, motion, detector=self.detector, verbose=False
        )
        ...
        print(
            f"    Internal Scherrer FWHM: mean={np.mean(scherrer_fwhm_deg):.4f}°, "
        ...
        pattern, _ = self.detector.render(peaks_dict, frames_to_render=0, method="nano")
        ...
        twotheta_result = np.degrees(
            2 * np.arcsin(Q_result * self.wavelength / (4 * np.pi))
        )

        # Find peaks in pattern
        found_Q, found_I = find_peaks_in_pattern(
            Q_result, I_result, prominence_fraction=0.005
        )
        ...
        for i, idx in enumerate(sorted_indices):
            if i >= 10:
                break
            Q_found = found_Q[idx]
            peak_twotheta = np.degrees(
                2 * np.arcsin(Q_found * self.wavelength / (4 * np.pi))
            )

            # Extract region around peak
            xrange = [
                max(x_data[0], peak_twotheta - 1.5),
            ...
            fit_result = fit_voigt_peak(
                x_segment, y_segment, peak_twotheta, verbose=False
            )
            ...
            if fit_result is not None and fit_result["success"]:
                ...
                # Calculate crystallite size using Scherrer equation
                size_nm = scherrer_from_fwhm(
                    fwhm_rad, self.wavelength, theta_rad, K=0.9
                )
        ...
        if len(estimated_sizes_voigt) > 0:
            ...
            std_estimated = (
                np.std(estimated_sizes_voigt) if len(estimated_sizes_voigt) > 1 else 0
            )
            error_percent = (
                abs(avg_estimated - expected_size_nm) / expected_size_nm * 100
            )
        ...
        im = ax1.imshow(
            ...
            vmin=0,
            vmax=(
                np.percentile(pattern_np[pattern_np > 0], 99)
                if np.any(pattern_np > 0)
                else 1
            ),
        )
        ax1.set_title(
            f"2D Diffraction Pattern\n(Target: {target_size_nm} nm crystallite)",
            fontsize=12,
        )
        ...
        ax2 = axes[0, 1]
        ax2.plot(
            twotheta_result, I_result, "b-", linewidth=0.8, label="Integrated pattern"
        )

        # Mark found peaks
        for info in peak_info:
            ax2.axvline(
                info["twotheta"], color="r", linestyle="--", alpha=0.5, linewidth=0.5
            )
...
"""
        ...
        output_path = os.path.join(
            test_reports_dir, "crystallite_size_100nm_analysis.png"
        )
        ...
        # Assertions
        self.assertGreater(
            len(estimated_sizes), 0, "No crystallite sizes could be estimated!"
        )
