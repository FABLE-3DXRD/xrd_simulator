...
class TestBeam(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.xray_propagation_direction = np.random.rand(
            3,
        )
        ...
        self.polarization_vector = np.random.rand(
            3,
        )
        self.polarization_vector = (
            self.polarization_vector
            - np.dot(self.polarization_vector, self.xray_propagation_direction)
            * self.xray_propagation_direction
        )
        self.beam = Beam(
            self.beam_vertices,
    ...
    def test_contains(self):
        ...
        self.assertTrue(
            is_contained == 1, msg="Beam centroid appears to not be contained by beam."
        )

        is_contained = self.beam.contains(self.beam.centroid + 5.5)
        self.assertTrue(
            is_contained == 0, msg="Beam centroid appears to not be contained by beam."
        )

        points = np.random.rand(3, 10) * 0.1 + 0.5
        for ic in self.beam.contains(points):
            self.assertTrue(
                ic == 1, msg="Expected interior point of beam is not contained by beam."
            )
        ...
        for ic in self.beam.contains(points):
            self.assertTrue(
                ic == 0,
                msg="Expected exterior point of beam is appears to be contained by beam.",
            )
    ...
    def test_intersect(self):
        ...
        for i in range(self.beam.vertices.shape[0]):
            ...
            self.assertAlmostEqual(
                np.min(np.linalg.norm(v1 - self.beam.vertices, axis=1)), 0
            )
    ...
    def test__get_proximity_intervals(self):
        ...
        self.beam_vertices[:, 1:] = (
            self.beam_vertices[:, 1:] / 10000000.0
        )  # tiny beam cross section
        ...
        tol = 2.0 / np.degrees(motion.rotation_angle)
        for i in range(sphere_centres.shape[0]):
            # far away sphere small radii approximation:
            fraction_before_beam_leaves_sphere = (
                np.arctan(sphere_radius[i] / np.linalg.norm(sphere_centres[i])) / np.pi
            )
            self.assertAlmostEqual(
                intervals[i][0][0], 0, msg="Proximity interval wrong"
            )
            self.assertLessEqual(
                np.abs(intervals[i][0][1] - fraction_before_beam_leaves_sphere[0]),
                tol,
                msg="Proximity interval wrong",
            )
            self.assertLessEqual(
                np.abs(
                    intervals[i][1][0] - (1.0 - fraction_before_beam_leaves_sphere[0])
                ),
                tol,
                msg="Proximity interval wrong",
            )
            self.assertAlmostEqual(
                intervals[i][1][1], 1.0, msg="Proximity interval wrong"
            )

        # Now with rotation and translation
        motion.translation = ensure_torch([-87.24, 34.6, 123.34])
        ...
        self.assertEqual(
            len(intervals[0]), 1, msg="Wrong number of proximity intervals"
        )
        self.assertEqual(
            len(intervals[1]), 1, msg="Wrong number of proximity intervals"
        )

        for i in range(sphere_centres.shape[0]):
            # search numerically for the point in time when the sphere leaves
            # the beam and compare to analytical intersection:
            times = torch.linspace(0.0, 1.0, 10000)
            cs = ensure_torch([motion(sphere_centres[i], t) for t in times])
            L = torch.sqrt(cs[:, 1] ** 2 + cs[:, 2] ** 2)
            fraction_before_beam_leaves_sphere = times[
                torch.argmin(torch.abs(L - sphere_radius[i]))
            ]
            self.assertAlmostEqual(
                intervals[i][0][0], 0, msg="Proximity interval wrong"
            )
            self.assertTrue(
                torch.abs(intervals[i][0][1] - fraction_before_beam_leaves_sphere)
                < tol,
                msg="Proximity interval wrong",
            )
    ...
    def test_set_beam_vertices(self):
        ...
        self.assertTrue(
            np.allclose(np.mean(new_vertices, axis=0), self.beam.centroid),
