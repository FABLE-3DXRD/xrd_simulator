...
class TestBeam(unittest.TestCase):
    ...
    def test_contains(self):
        is_contained = self.beam._contains(self.beam.centroid)
        self.assertTrue(
            is_contained.item() == 1,
            msg="Beam centroid appears to not be contained by beam.",
        )

        is_contained = self.beam._contains(self.beam.centroid + 5.5)
        self.assertTrue(
            is_contained.item() == 0,
            msg="Beam centroid appears to not be contained by beam.",
        )
        ...
        for ic in self.beam._contains(points):
            self.assertTrue(
                ic.item() == 1,
    ...
    def test_intersect(self):
        ...
        ch = self.beam._intersect(vertices)  # ConvexHull needs numpy array
        ...
        random_points = (torch.rand(25, 3) / 2.0 + 20.0)
        ch1 = ConvexHull(random_points)
        vertices = ensure_torch(ch1.points[ch1.vertices])
        ch = self.beam._intersect(vertices)
