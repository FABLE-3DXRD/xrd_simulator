...
class TestLaue(unittest.TestCase):
    ...
    def test_get_G_and_bragg_angle(self):
        ...
        G = laue._get_G(U, B, G_hkl=torch.tensor([[1, 2, -1], [1, 3, -1]], dtype=torch.float64))
        ...
        for i,angle in enumerate(theta):
            ...
            self.assertLessEqual(float(theta[i]), np.pi, msg="Bragg angle too large")
            self.assertGreaterEqual(float(theta[i]), 0, msg="Bragg angle is negative")
    ...
    def test_get_sin_theta_and_norm_G(self):
        ...
        for i,angles in enumerate(theta):
            self.assertAlmostEqual(
                float(sinth[i]), float(torch.sin(
                    theta[i])), msg="error theta")
            self.assertAlmostEqual(float(Gnorm[i]), float(torch.linalg.norm(G[:, i])), msg="error in norm of G",places=6)
    ...
    def test_find_solutions_to_tangens_half_angle_equation(self):
        ...
        G_0 = laue._get_G(U, B, G_hkl=torch.tensor([-1, 0, 0], dtype=torch.float64)).reshape(3,1)

        rx, ry, rz = rotation_axis
        K = torch.tensor([[0, -rz, ry],
                        [rz, 0, -rx],
                        [-ry, rx, 0]], dtype=torch.float64)
        rho_0_factor = -torch.matmul(k, torch.matmul(K, K)) # The operations involving G_0 are moved inside find_solutions_to_tangens...
        rho_1_factor = torch.matmul(k, K) # The operations involving G_0 are moved inside find_solutions_to_tangens...
        rho_2_factor = torch.matmul(k, torch.eye(3, 3, dtype=torch.float64) + torch.matmul(K, K))  # The operations involving G_0 are moved inside find_solutions_to_tangens...
        


        # Now G_0 and rho_factors are sent before computation to save memory when diffracting many grains.
        grains, planes, time_values, G = laue._find_solutions_to_tangens_half_angle_equation(G_0,rho_0_factor, rho_1_factor, rho_2_factor, rotation_angle)
        
        G_0 = G_0[torch.newaxis,:,:]
        rho_0 = torch.matmul(rho_0_factor,G_0)
        rho_2 = torch.matmul(rho_2_factor,G_0)+ torch.sum((G_0 * G_0), axis=1) / 2.   
        ...
        for t in time_values:
            ...
            self.assertTrue((not torch.isnan(t)),
            ...
            if not torch.isnan(t):
                ...
                t = torch.tan(t * rotation_angle / 2.)
                equation = (rho_2 - rho_0) * t**2 + 2 * rho_1 *t + (rho_0 + rho_2)
                self.assertAlmostEqual(float(equation.item()), 0, msg="Parametric solution wrong")
