...
"""Configure device (single decision) and set default dtype.

This script makes a single call to `cuda.configure_device()` to decide
whether to use CUDA. Do NOT perform any runtime backend switching or
CPU fallbacks here; keep all objects on the selected device.
"""
...
# orientation = np.repeat(orientation, mesh.number_of_elements, axis=0)


possible_phases = [
    phases.a_Ferrite
]  # ,phases.Austenite,phases.Cementite,phases.Oleg,phases.hcp]
